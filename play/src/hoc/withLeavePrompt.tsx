...
function withLeavePrompt(WrappedComponent: any, offsetTop = 0) {
  return function (props: any) {
    const history = useHistory()

    const editingRef = useRef<string[]>([])

    const addEditing = (title: string) => {
      editingRef.current.push(title)
    }

    const removeEditing = (title: string) => {
      editingRef.current = editingRef.current.filter((t) => t !== title)
    }

    const onPrompt = (nextLocation: any, action: any) => {
